...
//modal window
	...
	{
		var rg_url = /^http(s)?:\/\/youtu\.be\/[^&?]+$/, rg_url_1 = /^http(s)?:\/\/(www\.)?youtube\.com\/watch\?v=[^&?]+(&.+)?$/;  
		if(rg_url.test(url_give) || rg_url_1.test(url_give)) 
		return true;
		else return false;
	}
	...
	var checkpoint=function()
	{
		...
		if($('input[name="is_video"]').is(':checked')) infos.content=checkvideo(infos.content)?infos.content:null;
